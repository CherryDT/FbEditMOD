...
FasmDestroyString endp

FasmDestroyCmntBlock proc uses esi,lpMem:DWORD
...
FasmDestroyCmntBlock endp

FasmDestroyCommentsStrings proc uses esi,lpMem:DWORD

	mov		esi,lpMem
	mov		ecx,';'
	mov		edx,dword ptr szFasmString
	.while byte ptr [esi]
		.if byte ptr [esi]==cl
			invoke DestroyToEol,esi
			mov		esi,eax
...
FasmDestroyCommentsStrings endp

FasmPreParse proc uses esi,lpMem:DWORD

	invoke FasmDestroyCmntBlock,lpMem
	invoke FasmDestroyCommentsStrings,lpMem
	ret

FasmPreParse endp

FasmSkipLine proc uses esi,lpMem:DWORD,lpnpos:DWORD
...
FasmGetWord proc uses esi,lpMem:DWORD,lpnpos:DWORD
...
FasmGetWordSkip proc uses esi,lpMem:DWORD,lpnpos:DWORD
...
FasmGetWordSkip endp

FasmWhatIsIt proc uses esi,lpWord1:DWORD,len1:DWORD,lpWord2:DWORD,len2:DWORD
...
FasmIsIgnore proc uses ecx esi,nType:DWORD,len:DWORD,lpWord:DWORD
...
FasmFixUnknown proc uses ebx esi edi
...
FasmParseFile proc uses ebx esi edi,nOwner:DWORD,lpMem:DWORD
	LOCAL	lpword1:DWORD
	LOCAL	len1:DWORD
	LOCAL	lpword2:DWORD
	LOCAL	len2:DWORD
	...
	.while byte ptr [esi]
		...
		mov		lpdatatype,0
	  Nxtwrd:
		invoke FasmGetWord,esi,addr npos
		mov		esi,edx
		.if ecx
			invoke FasmIsIgnore,IGNORE_LINEFIRSTWORD,ecx,esi
			.if eax
				jmp		Nxt
			.endif
			mov		lpword1,esi
			...
			mov		len2,ecx
			lea		esi,[esi+ecx]
			invoke FasmWhatIsIt,lpword1,len1,lpword2,len2
			.if eax
				mov		lpdef,eax
		...
		.endif
	  Nxt:
		invoke FasmSkipLine,esi,addr npos
		inc		npos
		mov		esi,eax
	.endw
	invoke FasmFixUnknown
	ret
...
SaveParam:
	...
	mov		esi,edx
	.if ecx
		invoke FasmIsIgnore,IGNORE_PROCPARAM,ecx,esi
		.if eax
			lea		esi,[esi+ecx]
			jmp		SaveParam
		.endif
...
ParseData1:
	...
	invoke FasmGetWord,esi,addr npos
	mov		esi,edx
;	invoke FasmIsIgnore,IGNORE_PTR,ecx,esi
;	.if eax
;		xor		eax,eax
;		retn
;	.endif
