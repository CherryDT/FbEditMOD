...
Function HexFindDlgProc(ByVal hWin As HWND,ByVal uMsg As UINT,ByVal wParam As WPARAM,ByVal lParam As LPARAM) As Integer
	...
	Select Case uMsg
		Case WM_INITDIALOG
			...
			If lParam Then
				PostMessage(hWin,WM_COMMAND,IDC_HEXBTN_REPLACE Or (BN_CLICKED Shl 16),0)
			EndIf
			...
			If Event=BN_CLICKED Then
				Select Case id
					...
					Case IDC_HEXBTN_REPLACEALL
						...
						EndIf
						While freshex<>-1
							SendMessage(hWin,WM_COMMAND,IDC_HEXBTN_REPLACE Or (BN_CLICKED Shl 16),0)
						Wend
						'
					Case IDC_HEXRBN_HEX
						' Set hex type
