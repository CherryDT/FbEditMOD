...
ExportInclude proc uses esi edi,hMem:DWORD
	...
	.while byte ptr [esi].INCLUDEMEM.szfile
		...
		.if !eax
			mov		fResourceh,TRUE
		.endif
		invoke SaveStr,edi,offset szINCLUDE                 ; "#include"
		add		edi,eax
		mov		al,' '
		stosb
	...
	.elseif eax==WM_NOTIFY
		...
		.if eax==hGrd
			...
			.elseif eax==GN_BUTTONCLICK
				...
				.if byte ptr [eax]
					invoke strcpy,addr buffer2,[esi].GRIDNOTIFY.lpdata
					.if buffer2=='<'
						lea		edx,buffer2
						mov		eax,edx
						inc		eax
						...
						mov		byte ptr [edx],0
						.if szSystemPath
							invoke strcpy,addr buffer,addr szSystemPath
