...
; **** MOD add (caution: hardcoded in ParseMenu, ParseMenuEx)
szCsrLeft               db 'Left',0         
szCsrRight              db 'Right',0
szCsrUp                 db 'Up',0
szCsrDown               db 'Down',0
...
MnuSaveAccel proc uses esi edi,nAccel:DWORD,lpDest:DWORD
	...
	mov		eax,nAccel
	movzx	eax,al
	.if eax>='A' && eax<='Z' || eax>='0' && eax<='9'  ; *** MOD add 0...9
		stosb
	; *** MOD add Right, Left, Up, Down	
	.elseif eax==VK_LEFT                              
		invoke SaveStr,edi,offset szCsrLeft
		add    edi,eax	
	.elseif eax==VK_RIGHT
		invoke SaveStr,edi,offset szCsrRight
		add    edi,eax
	.elseif eax==VK_UP
		invoke SaveStr,edi,offset szCsrUp
		add    edi,eax	
	.elseif eax==VK_DOWN
		invoke SaveStr,edi,offset szCsrDown
		add    edi,eax
	; =================		
