...
Function GetFileID(ByVal lpFile As ZString Ptr) As Integer
		...
		Case AIM_COMMAND
			...
			Select Case nInx
				Case nMnuToggle
					If lpHandles->hred<>0 And lpHandles->hred<>lpHandles->hres Then
						If GetWindowLong(lpHandles->hred,GWL_ID)<>IDC_HEXED Then
							nInx=GetFileID(@lpData->filename)
							nLn=GetLineNumber
		...
		Case AIM_FILEOPENNEW
			If lpHandles->hred<>0 And lpHandles->hred<>lpHandles->hres Then
				If GetWindowLong(lpHandles->hred,GWL_ID)<>IDC_HEXED Then
					...
					EndIf
					nInx=GetFileID(Cast(ZString Ptr,lParam))
					If nInx Then
