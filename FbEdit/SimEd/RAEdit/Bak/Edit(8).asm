...
DeleteChar proc uses ebx esi edi,hMem:DWORD,cp:DWORD
	...
	mov		edx,cp
	PrintDec cp
	invoke SendMessage, [ebx].EDIT.hwnd, WM_GETTEXTLENGTH,0,0
	PrintDec eax, "TextLen"
	xor		eax,eax
	.if edx<[ebx].EDIT.edta.topcp
		mov		[ebx].EDIT.edta.topyp,eax
		mov		[ebx].EDIT.edta.topln,eax
...
;    	PrintText "EXCEPTION"
	...
	.if al==0Dh
		PrintText "0Dh"
		mov		edi,esi
		PrintDec [esi].CHARS.len
		.if [esi].CHARS.len==1
			PrintText "DelLine"
			PrintDec [ebx].EDIT.lastline
			PrintDec [ebx].EDIT.line
			mov eax,[ebx].EDIT.lastline
			.if [ebx].EDIT.line < eax
