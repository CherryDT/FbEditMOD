...
DumpProc proc
	...
	movzx	eax,[esi].COFFSYMBOL.NumberOfAuxSymbols
	mov		NumberOfAuxSymbols,eax
	.if (StorageClass==IMAGE_SYM_CLASS_EXTERNAL || StorageClass==IMAGE_SYM_CLASS_STATIC) && nType==20h && SectionNumber>0
		mov		eax,[esi].COFFSYMBOL.Zeroes
		.if !eax
			mov		ecx,hMemFile
			mov		eax,[ecx].COFFHEADER.NumberOfSymbols
		...
		.endw
	.elseif StorageClass==IMAGE_SYM_CLASS_FUNCTION || StorageClass==IMAGE_SYM_CLASS_EXTERNAL
...
DumpGlobals proc uses ebx esi edi
	...
	.while ebx<edi
		...
		movzx	eax,[esi].COFFSYMBOL.NumberOfAuxSymbols
		mov		NumberOfAuxSymbols,eax
		.if ((StorageClass==IMAGE_SYM_CLASS_STATIC && nType==0) || (StorageClass==2 && nType==2000h)) && NumberOfAuxSymbols==0 && SectionNumber>0
			mov		eax,[esi].COFFSYMBOL.Zeroes
			.if !eax
				mov		ecx,hMemFile
				mov		eax,[ecx].COFFHEADER.NumberOfSymbols
			...
			.endif
			invoke wsprintf,addr szOutput,addr szCoffGlobal,addr buffer,[esi].COFFSYMBOL.Value,nType,StorageClass,SectionNumber
			invoke SendMessage,hEdt,EM_REPLACESEL,FALSE,addr szOutput
		.endif
		.while NumberOfAuxSymbols
			lea		esi,[esi+sizeof COFFSYMBOL]
	...
	.while ebx<edi
		...
		.if !edx
			...
			.while ebx<edi
				movzx	eax,[esi].COFFSYMBOL.NumberOfAuxSymbols
				mov		NumberOfAuxSymbols,eax
				movsx	eax,[esi].COFFSYMBOL.StorageClass
				movzx	edx,[esi].COFFSYMBOL.SectionNumber
				.if eax==IMAGE_SYM_CLASS_FILE
					;Filename
					lea		esi,[esi+sizeof COFFSYMBOL]
					inc		ebx
					.if NumberOfAuxSymbols
						invoke lstrcpy,addr dbgproc.szFile,addr [esi].COFFSYMBOL.szShortName
						mov		eax,NumberOfAuxSymbols
						add		ebx,eax
						mov		edx,sizeof COFFSYMBOL
						mul		edx
			...
			.if NumberOfAuxSymbols==1
				mov		eax,[esi].COFFAUX1.TagIndex
				mov		dbgproc.TagIndex,eax
				mov		eax,[esi].COFFAUX1.TotalSize
				mov		dbgproc.TotalSize,eax
				mov		eax,[esi].COFFAUX1.PointerToLinenumber
				mov		dbgproc.PointerToLinenumber,eax
				lea		esi,[esi+sizeof COFFSYMBOL]
				inc		ebx
				push	esi
				mov		eax,hMemFile
				mov		esi,[eax].COFFHEADER.PointerToSymbolTable
				add		esi,eax
				mov		eax,dbgproc.TagIndex
				mov		edx,sizeof COFFSYMBOL
				mul		edx
				lea		esi,[esi+eax]
				mov		eax,dword ptr [esi].COFFSYMBOL.szShortName
				and		eax,0FFFFFFh
				.if eax=='fb.'
					mov		eax,[esi].COFFSYMBOL.Value
					mov		dbgproc.bfad,eax
					.if [esi].COFFSYMBOL.NumberOfAuxSymbols==1
						lea		esi,[esi+sizeof COFFSYMBOL]
						movzx	eax,[esi].COFFAUX2.Linenumber
						mov		dbgproc.bfln,eax
					.endif
					lea		esi,[esi+sizeof COFFSYMBOL]
				.endif
				mov		eax,dword ptr [esi].COFFSYMBOL.szShortName
				...
				.if eax=='fe.'
					mov		eax,[esi].COFFSYMBOL.Value
					mov		dbgproc.efad,eax
					.if [esi].COFFSYMBOL.NumberOfAuxSymbols==1
						lea		esi,[esi+sizeof COFFSYMBOL]
						movzx	eax,[esi].COFFAUX2.Linenumber
						mov		dbgproc.efln,eax
					.endif
					lea		esi,[esi+sizeof COFFSYMBOL]
				.endif
				pop		esi
			...
			pop		ebx
		.else
			add		edx,dbgproc.bfln
			invoke wsprintf,addr szOutput,addr szCoffLinenumber,addr dbgproc.szFile,eax,edx
			invoke SendMessage,hEdt,EM_REPLACESEL,FALSE,addr szOutput
		.endif
