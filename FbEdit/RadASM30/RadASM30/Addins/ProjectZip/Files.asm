...
FileDir proc uses ebx esi edi,lpPth:DWORD
	...
	.if eax!=INVALID_HANDLE_VALUE
	  ...
	  Next:
		...
		mov		[esi].ZFILEINFO.dwSizeLow,eax
		mov		eax,wfd.nFileSizeHigh
		mov		[esi].ZFILEINFO.dwSizeHigh,eax
		mov		eax,wfd.ftLastWriteTime.dwLowDateTime
		mov		[esi].ZFILEINFO.dwTimeLow,eax
		mov		eax,wfd.ftLastWriteTime.dwHighDateTime
		mov		[esi].ZFILEINFO.dwTimeHigh,eax
		add		esi,sizeof ZFILEINFO
		;Any more matches?
  ...
  @@:
	...
	.if eax!=LB_ERR
		...
		.endif
		.if eax!='.'
			lea		edi,[esi].ZFILEINFO.dwTimeLow
