...
Sub UpdateHexEdOptions (ByVal hEdt As HWND)
	
	Dim EditorMode As Long    = Any 
    Dim HexFont    As HEFONT  = Any 
    
    If hEdt Then
        EditorMode = GetWindowLong (hEdt, GWL_ID)   

        If EditorMode = IDC_HEXED Then

    	    HexFont.hFont    = ah.rafnt.hFont
    	    HexFont.hLnrFont = ah.rafnt.hLnrFont
    	    
    	    SendMessage hEdt, HEM_SETFONT, Abs (edtfnt.size) * edtopt.ExtraLineSpace \ 4, Cast (LPARAM, @HexFont)
