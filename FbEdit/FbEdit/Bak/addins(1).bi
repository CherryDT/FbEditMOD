...
Type TABMEM
	...
	filestate		As Integer						' Changed state: 0 = clean, 1 = dirty
	ft				As FILETIME						' Filetime last saved
    locked          As BOOL                         ' TabLock TRUE, FALSE
End Type
...
Type ADDINHANDLES
	...
	hraresed		As HWND							' Handle of RAResEd window (resed_toolbar + hrareseddlg + resed_project/resed_property)
	hrareseddlg		As HWND							' Handle of RAResEd dialog (drawing pane of resed)
...
Type ADDINDATA
    ...
    FbcIncPath      As ZString * MAX_PATH           ' compiler default include path
    FbcLibPath      As ZString * MAX_PATH           ' compiler default library path
End Type
...
Type ADDINFUNCTIONS
	...
	HH_Help              As Sub      ()
	IsProjectFile        As Function (Byref sFile As ZString) As Integer
End Type
...
Declare Sub LoadAddins
Declare Sub FreeAddins
Declare Function CallAddins (ByVal hWin As HWND, ByVal uMsg As UINT, ByVal wParam As WPARAM, ByVal lParam As LPARAM, ByVal hook1 As UINT) As Integer
Declare Function AddinManagerProc (ByVal hWin As HWND,ByVal uMsg As UINT,ByVal wParam As WPARAM,ByVal lParam As LPARAM) As Integer
...
Common Shared ah As ADDINHANDLES
Common Shared ad As ADDINDATA
Common Shared af As ADDINFUNCTIONS
