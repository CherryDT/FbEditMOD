	...
	Select Case uMsg
		Case WM_INITDIALOG
			...
			While nInx<=64
				...
				SetZStrEmpty (buff)             'MOD 26.1.2012 (speed)
...
Function TabOpt5Proc(ByVal hWin As HWND,ByVal uMsg As UINT,ByVal wParam As WPARAM,ByVal lParam As LPARAM) As Integer
	...
	Select Case uMsg
		Case WM_INITDIALOG
			...
			nInx=1
			While nInx<=32
				SetZStrEmpty (sType)             'MOD 26.1.2012 (speed)
				fbrstype.nid=0
...
Function TabOptionsProc(ByVal hWin As HWND,ByVal uMsg As UINT,ByVal wParam As WPARAM,ByVal lParam As LPARAM) As Integer
	...
	Select Case uMsg
		Case WM_INITDIALOG
			...
			hTabDlg(0)=CreateDialogParam(hInstance,Cast(ZString Ptr,IDD_TABOPT1),hTabOpt,@TabOpt1Proc,0)  ' Name Export  
			hTabDlg(1)=CreateDialogParam(hInstance,Cast(ZString Ptr,IDD_TABOPT3),hTabOpt,@TabOpt3Proc,0)  ' Grid Options
			hTabDlg(2)=CreateDialogParam(hInstance,Cast(ZString Ptr,IDD_TABOPT2),hTabOpt,@TabOpt2Proc,0)  ' Custom Controls
			hTabDlg(3)=CreateDialogParam(hInstance,Cast(ZString Ptr,IDD_TABOPT4),hTabOpt,@TabOpt4Proc,0)  ' Custom Styles
			hTabDlg(4)=CreateDialogParam(hInstance,Cast(ZString Ptr,IDD_TABOPT5),hTabOpt,@TabOpt5Proc,0)  ' Resource Types
			SelTab=0
			'
		Case WM_NOTIFY
