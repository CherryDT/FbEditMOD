...
Dim Shared lpOldEditProc    As WNDPROC
Dim Shared lpOldParEditProc As WNDPROC
...
Sub UpdateEditOption(ByVal hEdt As HWND)
	Dim style As Integer
	
	SendMessage(hEdt,REM_SETCOLOR,0,Cast(Integer,@fbcol.racol))
	' MOD 19.12.2011
	SendMessage hEdt, REM_SETFONT, Abs (edtfnt.size) * edtopt.ExtraLineSpace \ 4, Cast (LPARAM, @ah.rafnt)
	'===========================
...
Function IsEditorWindow (ByVal hWin As HWND, ByVal EdTyp As EditorType) As BOOL
    
    Dim EditorMode As Long = Any 
    
    If hWin Then
        EditorMode = GetWindowLong (hWin, GWL_ID)   
        DebugPrint (EditorMode) 
        DebugPrint (EditorMode = IDC_CODEED)
        DebugPrint (EditorMode = IDC_TEXTED)
        DebugPrint ((EditorMode = IDC_CODEED) OrElse (EditorMode = IDC_TEXTED))
        Select Case As Const EdTyp
