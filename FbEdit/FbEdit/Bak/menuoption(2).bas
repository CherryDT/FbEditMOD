...
Type  EntryName As ZString * 32
Type  EntryItem As ZString * 256   
Const EntrySize As Integer = SizeOf (EntryName) + 1 + SizeOf (EntryItem)      ' 1 Delimiter
...
Sub MakeSubMenu (ByVal SubMenuID As UINT, ByVal FirstID As UINT, ByVal LastID As UINT, ByRef IniSection As ZString Ptr)
	...
	For ID = FirstID To LastID
		...
		GetPrivateProfileString IniSection, Str (ID - FirstID + 1), NULL, @buff, EntrySize, @ad.IniFile
...
Sub EditUpdate(ByVal hWin As HWND)
	...
	If fUpdate Then
		GetDlgItemText hWin, IDC_EDTMEITEM, @buff, SizeOf (EntryName)
		TrimWhiteSpace buff
		ZStrReplaceChar @buff, Asc (","), Asc ("-")           ' forbidden char
		n = lstrlen (buff)
		buff[n] = VK_TAB
		GetDlgItemText hWin, IDC_EDTMECMND, @buff + n + 1, SizeOf (EntryItem)
...
Sub MenuOptionSave(ByVal hWin As HWND)
	...
	Case 3
		...
		GetPrivateProfileString    @"Make", @"Module",  @"Module Build,fbc -c", @sItem2, SizeOf (sItem2), @ad.IniFile
		WritePrivateProfileSection @"Make", @buff, @ad.IniFile
		WritePrivateProfileString  @"Make", @"fbcPath", @sItem,         @ad.IniFile
		WritePrivateProfileString  @"Make", @"Module",  @sItem2,        @ad.IniFile
		WritePrivateProfileString  @"Make", @"Current", Str (nInx + 1), @ad.IniFile
	Case 4
		nInx = SendDlgItemMessage (hWin, IDC_LSTME, LB_GETCURSEL, 0, 0)
		If nInx = LB_ERR Then nInx = 0
		x = GetPrivateProfileInt  (@"Make", @"Recompile", 0, @ad.ProjectFile)
		GetPrivateProfileString    @"Make", @"Module", NULL, @sItem, SizeOf (sItem), @ad.ProjectFile
		WritePrivateProfileSection @"Make", @buff, @ad.ProjectFile
		WritePrivateProfileString  @"Make", @"Module", @sItem, @ad.ProjectFile
	...
	Select Case uMsg
		Case WM_INITDIALOG
			...
			Do
				...
				Case 4     ' select make option for projects main file 
					GetPrivateProfileString(StrPtr("Make"),Str(i),NULL,@buff,SizeOf(sCmd),@ad.ProjectFile)
				Case 5     ' select make option to import (fbedit.ini -> project.ini)
					GetPrivateProfileString(StrPtr("Make"),Str(i),NULL,@buff,SizeOf(sCmd),@ad.IniFile)
