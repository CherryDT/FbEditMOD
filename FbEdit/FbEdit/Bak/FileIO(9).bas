...
Sub ReadTextFile(ByVal hWin As HWND,ByVal hFile As HANDLE,ByVal lpFilename As ZString Ptr)
	...
	'Dim szItem As ZString*260                                ' MOD 3.3.2012
	...
	nLastSize=SendMessage(hWin,WM_GETTEXTLENGTH,0,0)+1        ' ??? TODO  
	SendMessage(hWin,EM_SETMODIFY,FALSE,0)
	SendMessage(hWin,REM_SETCHANGEDSTATE,FALSE,0)
	'lstrcpy(@szItem,lpFilename)                              ' MOD 3.3.2012 remove
	
	'todo
	SetPropertyDirty hWin  
		
	
	If GetFBEFileType(*lpFilename)=FBFT_CODE Then             ' MOD 3.3.2012 szItem => *lpFilename
    ...
    If hWin Then                                                                ' MOD 1.3.2012   add 
    	...
    	If HFILE <> INVALID_HANDLE_VALUE Then
    		If hWin = ah.hres Then
    			...
    			'GlobalLock(hMem)                       ' MOD 3.2.2012    fixed memory lock count always zero
    			...
    			SendMessage ah.hraresed, PRO_OPEN, Cast (WPARAM, lpFile), Cast (LPARAM, hMem)
    		Else
